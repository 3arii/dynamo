...
def dtwd(traj1, traj2):
    ...
    for i in traj1:
        dtwd_value_1 += min(euclidean_distance(i, j) for j in traj2)
        count1 += 1  
    # Calculate the second summation term
    for j in traj2:
        dtwd_value_2 += min(euclidean_distance(i, j) for i in traj1)
        count2 += 1
    # Normalize by the total number of points considered
    normalized_dtwd = (dtwd_value_1 / count1) + (dtwd_value_2 / count2)
            ...
            if epoch % 10 == 0:
                with torch.no_grad():
                    ...
                    for i in range(len(data.train_data)):
                        ...
                        traj1 = iflow.generate_trj(torch.Tensor(data.train_data[i][0]).unsqueeze(0), T=150) 
                        traj2 = data.train_data[0][i]

                        normalized_dtwd_value = dtwd(traj1, traj2)
                        dtwd_list.append(normalized_dtwd_value)
                    
                    print(dtwd_list)
                    print(np.mean(dtwd_list))
